...
export default class ShareAsGistPlugin extends Plugin {
    ...
    this.addCommand({
      ...
      editorCallback: async (editor: Editor, view: MarkdownView) => {
        if (!this.settings.dotcomPersonalAccessToken) {
          return new Notice('You need to add your GitHub personal access token in Settings.')
        }
  ...
  async createGist(filename: string, content: string, isPublic: boolean): Promise<CreateGistResult> {
    try {
...
class ShareAsGistSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc('An access token for GitHub.com with permission to write gists. You can create one from "Settings" in your GitHub account.')
			.addText(text => text
